...
export default function SuccessDashboard() {
    ...
    const timeout = setTimeout(() => {
      ...
      Promise.all([
        authFetch(`/api/goal-signals?action=stats&days=${days}`)
          ...
          .then(setSignals)
          .catch(() => {}),
        authFetch(`/api/admin/pnl?days=${days}`)
          .then((r) => r.json() as Promise<PnlStats>)
          .then(setPnl)
          .catch(() => {}),
  ...
  const accuracyRate = overall?.accuracyRate ?? 0;
  const brierScore = overall?.brierScore ?? 1;
  const dailyMetrics = signals?.dailyMetrics ?? [];
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black text-gray-800">
            📊 Başarı Gösterge Paneli
          </h1>
          <p className="text-xs text-gray-500 mt-0.5">
            Sinyal başarısı · Model durumu · Kâr/ROI · Günlük trend
          </p>
        </div>
        <div className="flex gap-1">
          {[7, 14, 30, 90].map((d) => (
            <button
              key={d}
              onClick={() => setDays(d)}
              className={`px-3 py-1.5 text-xs font-bold rounded-lg border-2 ${days === d ? "border-indigo-400 bg-indigo-50 text-indigo-700" : "border-gray-200 text-gray-600"}`}
            >
              {d}g
            </button>
          ))}
      ...
      {!loading && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <KPIBox
              label="Toplam Sinyal"
              value={fmt(overall?.totalSignals ?? 0, 0)}
              sub={signal(overall?.accuracyRate)}
              color={accuracyRate > 0.6 ? "#10b981" : "#f59e0b"}
            />
            <KPIBox
              label="Doğruluk"
              value={fmtPct(overall?.accuracyRate)}
              sub={
                signal(overall?.accuracyRate) +
                (accuracyRate > 0.6 ? " ✓" : " ⚠")
              }
              color={accuracyRate > 0.6 ? "#10b981" : "#f59e0b"}
            />
            ...
            />
            <KPIBox
              label="Gol Sinyal Oranı"
              value={fmtPct(overall?.goalAfterSignalRate)}
              sub={`${overall?.signalsWithGoal ?? 0}/${overall?.totalSignals ?? 0}`}
              color="#8b5cf6"
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {champ &&
              ["xgb", "gbdt", "inplay", "gap", "pi", "glicko2"].map((n) => {
                const m = champ[n];
                if (!m) return null;
                return (
                  <div
                        ...
                        return (
                          ...
                          >
                            ...
                            <td
                              ...
                              style={{
                                color: goalRate > 0.3 ? "#10b981" : "#6b7280",
                              }}
                            >
                              {fmtPct(goalRate)}
                            </td>
          ...
          {pnlStats && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <KPIBox
                label="Toplam Kâr"
                value={`${pnlStats.totalPnl > 0 ? "+" : ""}${fmt(pnlStats.totalPnl)} birim`}
                sub={`${pnlStats.totalSignals} sinyalde`}
                color={pnlStats.totalPnl > 0 ? "#10b981" : "#ef4444"}
              />
              <KPIBox
                label="Win Rate"
                value={fmtPct(pnlStats.winRate)}
                sub={`${pnlStats.won}/${pnlStats.totalSignals}`}
                color={pnlStats.winRate > 0.5 ? "#10b981" : "#f59e0b"}
              />
              <KPIBox
                label="ROI"
                value={`${pnlStats.roi >= 0 ? "+" : ""}${fmt(pnlStats.roi)}%`}
                sub="Yatırılan/Geri Dönen"
                color={pnlStats.roi > 0 ? "#10b981" : "#ef4444"}
              />
              <KPIBox
                label="Sharpe"
                value={fmt(pnlStats.sharpeRatio)}
                sub={pnlStats.sharpeRatio > 1 ? "✅ İyi" : "⚠️ Zayıf"}
                color={pnlStats.sharpeRatio > 1 ? "#10b981" : "#f59e0b"}
              />
            </div>
          )}
...
function KPIBox({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
      ...
      <div className="text-lg font-black" style={{ color: color ?? "#374151" }}>
        {value}
      </div>
