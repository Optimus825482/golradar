...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (action === "momentum") {
      ...
      if (!momentum) {
        return NextResponse.json(
          { error: "Momentum data not available for this match" },
          { status: 404 },
        );
      }
    ...
    if (action === "oddsMovement") {
      ...
      if (!odds) {
        return NextResponse.json(
          { error: "Odds data not available for this match" },
          { status: 404 },
        );
      }
    ...
    if (action === "backtestMatches") {
      ...
      const maxMatches = Math.min(
        200,
        Math.max(1, parseInt(searchParams.get("maxMatches") || "20", 10)),
      );
      const enrich = searchParams.get("enrich") === "true"; // Fetch momentum+events+odds for each match

      const { fetchGoalooMatchesRecent, enrichGoalooMatch } =
        await import("@/lib/goaloo");

      logInfo(
        "Goaloo API",
        `Fetching backtest matches: daysBack=${daysBack}, maxMatches=${maxMatches}, enrich=${enrich}`,
      );
    ...
    if (action === "resolve") {
      ...
      if (!home || !away || !date) {
        return NextResponse.json(
          { error: "home, away, and date parameters required" },
          { status: 400 },
        );
      }
    ...
    return NextResponse.json(
      {
        error:
          "Unknown action. Use: matches, momentum, events, odds, oddsMovement, teamstats, backtestMatches, resolve",
      },
      { status: 400 },
    );
  } catch (error: any) {
