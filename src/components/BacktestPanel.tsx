...
export default function BacktestPanel() {
  ...
  const [signalStats, setSignalStats] = useState<SignalAccuracyStats | null>(
    null,
  );
  ...
  const fetchSignalData = useCallback(async () => {
    try {
      ...
      if (sigResp.ok) {
        const sigData = (await sigResp.json()) as SignalAccuracyStats;
        setSignalStats((prev) => {
          setHasNewSignals(
            Boolean(prev && sigData.totalSignals > prev.totalSignals),
          );
          return sigData;
        });
      }
  ...
  const resolvedCount = signalStats
    ? signalStats.signalsWithGoal + signalStats.signalsWithoutGoal
    : 0;
      ...
      <div className="bg-linear-to-br from-indigo-50 to-purple-50 rounded-xl border border-indigo-200 shadow-sm overflow-hidden">
        <div className="px-4 pt-3 pb-2">
          ...
          <p className="text-[10px] text-indigo-600 mt-1">
            Biten maclarin istatistiklerini kullanarak Goal Radar algoritmasini
            geriye donuk test eder. Gercek mac verileri uzerinde calisir —
            sentetik/test verisi kullanmaz.
          </p>
        </div>
        ...
        <div className="px-4 py-3 space-y-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-1.5">
              <label className="text-[10px] font-medium text-indigo-700">
                Gun:
              </label>
              <select
                value={simDaysBack}
            ...
            <div className="bg-white/80 rounded-lg p-3 border border-indigo-100">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold text-indigo-700">
                  {simulating
                    ? "Simulasyon Devam Ediyor..."
                    : "Simulasyon Tamamlandi!"}
                </span>
                <span className="text-[10px] text-indigo-500 font-mono">
                  {simProgress.percentComplete}% ·{" "}
                  {formatElapsed(simProgress.elapsedMs)}
                </span>
              </div>
              ...
              {simProgress.errors > 0 && (
                <div className="mt-1 text-[9px] text-amber-600">
                  {simProgress.errors} hata · {simProgress.matchesWithoutStats}{" "}
                  istatistiksiz mac
                </div>
              )}
            </div>
            ...
            <div className="bg-white/60 rounded-lg p-2.5 border border-indigo-100">
              <div className="text-[9px] text-indigo-600 space-y-1">
                ...
                <div className="flex items-start gap-1.5">
                  ...
                  <span>
                    HT/FT istatistiklerinden sentetik snapshot olustur (5dk
                    aralik)
                  </span>
                </div>
                <div className="flex items-start gap-1.5">
                  ...
                  <span>
                    Her dakikada 12-faktorlu Goal Radar algoritmasini calistir
                  </span>
                </div>
                <div className="flex items-start gap-1.5">
                  <span className="text-indigo-400">4.</span>
                  <span>
                    Sinyal tespit edildiginde kaydet, gercekte gol olup
                ...
                <div className="flex items-start gap-1.5">
                  ...
                  <span>
                    Brier skoru, kalibrasyon, faktor analizi ile sonuclari
                    raporla
                  </span>
                </div>
              </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-4 pt-3 pb-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <div
                className={`w-1.5 h-1.5 rounded-full ${autoRefresh ? "bg-emerald-500 animate-pulse" : "bg-gray-300"}`}
              />
        ...
        <div className="px-4 py-2 bg-linear-to-r from-slate-50 to-gray-50 border-t border-gray-100 flex items-center justify-between text-[9px]">
          <div className="flex items-center gap-3">
            ...
            {hasNewSignals && (
              <span className="bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded font-bold">
                Yeni!
              </span>
            )}
          </div>
      ...
      {signalStats && signalCount > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          <QuickCard
            ...
            color={
              signalStats.goalAfterSignalRate >= 40
                ? "text-emerald-600"
                : signalStats.goalAfterSignalRate >= 25
                  ? "text-amber-600"
                  : "text-red-500"
            }
            bg={
          ...
          <QuickCard
            ...
            color={
              signalStats.accuracyRate >= 70
                ? "text-emerald-600"
                : signalStats.accuracyRate >= 50
                  ? "text-amber-600"
                  : "text-red-500"
            }
            bg="bg-white"
              ...
              <div className="flex flex-col gap-2 w-full max-w-sm">
                ...
                >
                  ...
                  <span className="text-[9px] font-normal opacity-80">
                    (Biten maclar uzerinde)
                  </span>
                </button>
                <div className="text-[9px] text-gray-400 text-center">
                  Veya canli mac izlenirken %55+ gol ihtimali tespit edildiginde
                  otomatik kaydedilir
                </div>
              </div>
            </div>
      ...
      {backtestData && backtestData.signalCount > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            {[
              ...
              >
                ...
                {tab.key === "signals" && pendingCount > 0 && (
                  <span className="bg-amber-400 text-white text-[7px] px-1 rounded-full font-bold">
                    {pendingCount}
                  </span>
                )}
              </button>
          ...
          <div className="px-4 py-3">
            {activeTab === "overview" && (
              <OverviewTab bt={backtestData} stats={signalStats} />
            )}
            {activeTab === "signals" && (
              <SignalsTab signals={recentSignals} stats={signalStats} />
            )}
            {activeTab === "calibration" && (
              <CalibrationTab bt={backtestData} />
            )}
