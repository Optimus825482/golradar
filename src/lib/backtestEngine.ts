...
import {
  ...
  computeSignalDecay,
  computeScoreDifferenceAccuracy,
  computeFalsePositivePatterns,
...
const BACKTEST_DIR = sBe
  ? sBe.join(process.cwd(), "data", "backtest-results")
  : "";
...
function loadSignalsFromFlatFiles(
  startDate?: string,
  endDate?: string,
): SignalRecord[] {
  const fs = getBeFs();
...
export async function loadSignalsForRangeAsync(
  startDate?: string,
  endDate?: string,
): Promise<SignalRecord[]> {
  const flat = loadSignalsFromFlatFiles(startDate, endDate);
  ...
  if (fromDB.length > 0) {
    logInfo(
      "BacktestEngine",
      `Flat files empty, loaded ${fromDB.length} signals from DB`,
    );
  }
  return fromDB;
}
...
export function runBacktest(config: BacktestConfig = {}): BacktestResult {
  return runBacktestImpl(config) as unknown as BacktestResult;
}
...
export async function runBacktestAsync(
  ...
  const signals: any[] = await loadSignalsForRangeAsync(
    config.startDate,
    config.endDate,
  );
...
function runBacktestImplCore(signals: any[], config: BacktestConfig = {}): any {
  ...
  if (signals.length === 0) {
    return {
      ...
      factorImportance: [],
      brierScore: 0,
      brierDecomposition: { refinement: 0, calibration: 0, uncertainty: 0 },
      sideAccuracy: {
        home: { correct: 0, total: 0, accuracy: 0 },
        away: { correct: 0, total: 0, accuracy: 0 },
      },
      escalation: {
        early: { correct: 0, total: 0, accuracy: 0 },
        late: { correct: 0, total: 0, accuracy: 0 },
      },
      dailyPerformance: [],
    };
  }
  ...
  for (const s of signals) {
    ...
    if (isPos && outcome) {
      tp++;
      scoreSumCorrect += s.score;
      scoreCorrectN++;
    } else if (isPos && !outcome) {
      fp++;
      scoreSumIncorrect += s.score;
      scoreIncorrectN++;
  ...
  const thresholds: any = thresholdRange.map((threshold) => {
    ...
    return {
      ...
      f1:
        tprec + trec > 0
          ? Math.round(((2 * tprec * trec) / (tprec + trec)) * 1000) / 1000
          : 0,
      accuracy:
        ttp + tfp + ttn + tfn > 0
          ? Math.round(((ttp + ttn) / (ttp + tfp + ttn + tfn)) * 1000) / 1000
          : 0,
    };
  });
  ...
  for (let i = 0; i < bucketCount; i++) {
    ...
    buckets.push({
      ...
      observedRate:
        bin.length > 0
          ? bin.filter((s) => s.goalHappened).length / bin.length
          : 0,
    });
  }
      ...
      if (fs && path && BACKTEST_DIR) {
        ...
        const payload = {
          ...
          result: {
            ...
            brierDecomposition: (computeBrierDecomposition as any)(
              signals as any,
            ),
  ...
  return {
    ...
    avgScoreCorrect:
      scoreCorrectN > 0
        ? Math.round((scoreSumCorrect / scoreCorrectN) * 10) / 10
        : 0,
    avgScoreIncorrect:
      scoreIncorrectN > 0
        ? Math.round((scoreSumIncorrect / scoreIncorrectN) * 10) / 10
        : 0,
    thresholds,
    buckets,
    calibrationCurve: computeCalibrationCurve(signals, bucketCount),
...
function computeSideAccuracy(signals: any[]): {
  ...
  for (const s of signals) {
    if (s.side === "home") {
      ht++;
      if (s.goalHappened) hc++;
    } else if (s.side === "away") {
      at++;
      if (s.goalHappened) ac++;
    }
  }
  return {
    home: {
      correct: hc,
      total: ht,
      accuracy: ht > 0 ? Math.round((hc / ht) * 1000) / 1000 : 0,
    },
    away: {
      correct: ac,
      total: at,
      accuracy: at > 0 ? Math.round((ac / at) * 1000) / 1000 : 0,
    },
  };
}

function computeEscalationAccuracy(signals: any[]): {
  early: { correct: number; total: number; accuracy: number };
  late: { correct: number; total: number; accuracy: number };
} {
  let ec = 0,
    et = 0,
    lc = 0,
  ...
  for (const s of signals) {
    if (s.score < 70) {
      et++;
      if (s.goalHappened) ec++;
    } else {
      lt++;
      if (s.goalHappened) lc++;
    }
  }
  return {
    early: {
      correct: ec,
      total: et,
      accuracy: et > 0 ? Math.round((ec / et) * 1000) / 1000 : 0,
    },
    late: {
      correct: lc,
      total: lt,
      accuracy: lt > 0 ? Math.round((lc / lt) * 1000) / 1000 : 0,
    },
  };
}
